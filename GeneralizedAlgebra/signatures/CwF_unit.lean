...
def ℭ𝔴𝔉₁ : GAT := ⦃
    ...
    unit : (Γ : Con) ⇒ Ty Γ,
    unit_stab : (Δ : Con) ⇒ (Γ : Con) ⇒ (σ : Sub Δ Γ) ⇒
        substTy Δ Γ σ (unit Γ) ≡ unit Δ,
    star : (Γ : Con) ⇒ Tm Γ (unit Γ),
    star_stab : (Δ : Con) ⇒ (Γ : Con) ⇒ (σ : Sub Δ Γ) ⇒
        substTm Δ Γ (unit Γ) σ (star Γ)   #⟨unit_stab Δ Γ σ⟩
        ≡ star Δ,
    unit_elim : (Γ : Con) ⇒
        (M : Ty (ext Γ (unit Γ))) ⇒
        (m : Tm Γ (substTy Γ (ext Γ (unit Γ)) (pair Γ Γ (unit Γ) (id Γ) (star Γ)) M)) ⇒
        Tm (ext Γ (unit Γ)) M,
    unit_β : (Γ : Con) ⇒
        (M : Ty (ext Γ (unit Γ))) ⇒
        (m : Tm Γ (substTy Γ (ext Γ (unit Γ)) (pair Γ Γ (unit Γ) (id Γ) (star Γ)) M)) ⇒
        substTm Γ (ext Γ (unit Γ)) M (pair Γ Γ (unit Γ) (id Γ) (star Γ)) (unit_elim Γ M m)
        ≡ m
⦄
